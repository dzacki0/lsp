...
if (isset($_POST['tambah'])) {
    ...
    if ($query) {
        ...
        echo "<script>alert('Gagal menambahkan menu'); window.location.href = 'menu.php';</script>";
    }
}
...
if (isset($_POST['edit'])) {
    ...
    $query = mysqli_query($koneksi, "UPDATE menu SET nama_menu = '$nama', harga = '$harga', kategori = '$kategori' WHERE id = '$id'");

    if ($query) {
        echo "<script>alert('Menu berhasil diperbarui'); window.location.href = 'menu.php';</script>";
    } else {
        echo "<script>alert('Gagal memperbarui menu'); window.location.href = 'menu.php';</script>";
    }
}
...
if (isset($_GET['hapus'])) {
    ...
    $hapus_transaksi = mysqli_query($koneksi, "DELETE FROM transaksi WHERE id_menu = '$id'");
    
    if ($hapus_transaksi) {
        $hapus_menu = mysqli_query($koneksi, "DELETE FROM menu WHERE id = '$id'");
        
        if ($hapus_menu) {
            echo "<script>alert('Menu berhasil dihapus'); window.location.href = 'menu.php';</script>";
        } else {
            echo "<script>alert('Gagal menghapus menu'); window.location.href = 'menu.php';</script>";
        }
    } else {
        echo "<script>alert('Gagal menghapus transaksi terkait'); window.location.href = 'menu.php';</script>";
    }
}
            ...
            while ($row = mysqli_fetch_assoc($menu)) {
                echo "<tr>
                        ...
                        <td>
                            ...
                            <button class='btn btn-sm btn-warning' data-bs-toggle='modal' data-bs-target='#editModal' data-id='{$row['id']}' data-nama='{$row['nama_menu']}' data-harga='{$row['harga']}' data-kategori='{$row['kategori']}'>Edit</button>
                        </td>
                      </tr>";
                $no++;
            }
    ...
    <form method="post">
        <div class="modal-content">
          ...
          <div class="modal-body">
                <div class="mb-3">
                    <label>Nama Menu</label>
                    <input type="text" name="nama" id="editNama" required class="form-control">
                </div>
                <div class="mb-3">
                    <label>Harga</label>
                    <input type="number" name="harga" id="editHarga" required class="form-control">
                </div>
                <div class="mb-3">
                    <label>Kategori</label>
...
<script>
// Script to fill modal with the data of the selected menu
